...
{
	public static UserData Load() => JsonUtils.Load<UserData>(DataUtils.UserDataFile, () => UserData.Empty, IntegrityCheck);
...
public record PropFilter
{
	...
	public HtmlNode MkEditUI()
	{
		return Div("base-propfilter")/*.ClsOn("base-propfilter", userData.PropFilter_IsSel(this))*/.Wrap(
			new List<HtmlNode>()
			{
				Div("base-propfilter-name")
					...
					.OnClick(() =>
					{
						//userData.PropFilter_SetSel(May.Some(this));
					}, true),
				new HtmlNode("input")
	...
	private HtmlNode[] MkEditUIType() => Type switch
	{
		PropFilterType.YearMin or PropFilterType.YearMax => Arr(
			new HtmlNode("input").Cls("base-propfilter-main")
				...
				.HookArg("change", valStr =>
				{
					//userData.Filter_SingleSel_PropFilter_Change(this, this with { YearVal = int.Parse(valStr) });
				}, "this.value"),
			Div().Txt($"{YearVal}")
		),
		...
		PropFilterType.ImdbRatingMin or PropFilterType.ImdbRatingMax => Arr(
			new HtmlNode("input").Cls("base-propfilter-main")
				...
				.HookArg("change", valStr =>
				{
					//userData.Filter_SingleSel_PropFilter_Change(this, this with { ImdbRatingVal = int.Parse(valStr) });
				}, "this.value"),
		...
		PropFilterType.ReviewRatingMin or PropFilterType.ReviewRatingMax => Arr(
			new HtmlNode("input").Cls("base-propfilter-main")
				...
				.HookArg("change", valStr =>
				{
					//userData.Filter_SingleSel_PropFilter_Change(this, this with { ReviewRatingVal = int.Parse(valStr) });
				}, "this.value"),
		...
		PropFilterType.ReviewCountMin or PropFilterType.ReviewCountMax => Arr(
			new HtmlNode("input").Cls("base-propfilter-main")
				...
				.HookArg("change", valStr =>
				{
					//userData.Filter_SingleSel_PropFilter_Change(this, this with { ReviewCountVal = int.Parse(valStr) });
				}, "this.value"),
		...
		PropFilterType.TitleSearch => Arr(
			new HtmlNode("input").Cls("base-propfilter-main")
				...
				{
					//userData.Filter_SingleSel_PropFilter_Change(this, this with { TitleSearch = valStr });
				}, "this.value")
		),
		...
		PropFilterType.TitlePlotSearch => Arr(
			new HtmlNode("input").Cls("base-propfilter-main")
				...
				.HookArg("input", valStr =>
				{
					//userData.Filter_SingleSel_PropFilter_Change(this, this with { TitlePlotSearch = valStr });
				}, "this.value")
		),
		...
		PropFilterType.DirectorSearch => Arr(
			new HtmlNode("input").Cls("base-propfilter-main")
				...
				.HookArg("input", valStr =>
				{
					//userData.Filter_SingleSel_PropFilter_Change(this, this with { DirectorSearch = valStr });
				}, "this.value")
		),
		...
		PropFilterType.ActorSearch => Arr(
			new HtmlNode("input").Cls("base-propfilter-main")
				...
				.HookArg("input", valStr =>
				{
					//userData.Filter_SingleSel_PropFilter_Change(this, this with { ActorSearch = valStr });
				}, "this.value")
		),
